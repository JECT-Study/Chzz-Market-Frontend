...
vi.mocked(useGetImminentProducts).mockReturnValue({
  isImminentLoading: false, // 로딩 완료 상태로 설정
  imminentProducts,
});
vi.mocked(useGetPreRegisterProducts).mockReturnValue({
  isPreRegisterLoading: false, // 로딩 완료 상태로 설정
  preRegisterProducts,
});
...
describe('Home 테스트', () => {
  ...
  describe('경매 상품 테스트', () => {
    ...
    test('사전 등록 상품에 사진, 이름, 시작 가격을 표시하고 클릭하면 상품 상세 페이지로 이동한다.', async () => {
      ...
      const preRegisterProductItems = await screen.findAllByRole('figure', {
        name: /pre-register/,
      });

      const firstPreRegisterProduct = preRegisterProductItems[0];

      // 이미지 확인
      const imgElement = screen.getByRole('img', {
        name: '0_img_pre-register',
      });
      expect(firstPreRegisterProduct).toContainElement(imgElement);

      // 이름 확인
      const nameElement = screen.getByLabelText('0_name_pre-register');
      expect(nameElement).toHaveTextContent('[뉴발란스] 993');
      expect(firstPreRegisterProduct).toContainElement(nameElement);

      // 시작 가격 확인
      const priceElement = screen.getByLabelText('0_startPrice_pre-register');
      expect(priceElement).toHaveTextContent('230,000원');
      expect(firstPreRegisterProduct).toContainElement(priceElement);

      await user.click(firstPreRegisterProduct);
    ...
    test('mock data 개수의 경매 상품이 렌더링 된다.', async () => {
      ...
      // 마감 임박 경매 상품
      const imminentImages = await screen.findAllByRole('img', {
        name: /imminent/,
      });

      // 사전 등록 상품
      const preRegisterImages = await screen.findAllByRole('img', {
        name: /pre-register/,
      });

      expect(bestImages).toHaveLength(5);
      expect(preRegisterImages).toHaveLength(5);
      expect(imminentImages).toHaveLength(5);
    });
    ...
    describe('경매 시간은 남은 시간에 따라 다른 색으로 표시한다.', () => {
      test.only('8시간 미만일 경우 빨간색으로 표시한다.', async () => {
        ...
        // 7시간 남은 경매 상품
        const redTimeProduct = bestProductItems[1];

        const redTimeElement = screen.getByLabelText('1_timeLeft_best');
        expect(redTimeProduct).toContainElement(redTimeElement);
      ...
      test('16시간 미만일 경우 주황색으로 표시한다.', async () => {
        ...
        // 14시간 남은 경매 상품
        const orangeTimeProduct = bestProductItems[0];

        const orangeTimeElement = screen.getByLabelText('0_timeLeft_best');
        expect(orangeTimeProduct).toContainElement(orangeTimeElement);
      ...
      test('24시간 미만일 경우 초록색으로 표시한다.', async () => {
        ...
        // 7시간 남은 경매 상품
        const greenTimeProduct = bestProductItems[2];

        const greenTimeElement = screen.getByLabelText('2_timeLeft_best');
        expect(greenTimeProduct).toContainElement(greenTimeElement);
