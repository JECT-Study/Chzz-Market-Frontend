  ...
  return (
    <Layout>
      ...
      <Layout.Main>
        <div className='flex flex-col gap-8 pt-4'>
          ...
          <div className='flex items-center justify-between'>
            ...
            <button aria-label="필터링 버튼" onClick={handleFilterState} className='flex items-center gap-1 cursor-pointer text-body2 text-gray1'>
          ...
          <ul className='flex flex-col gap-2'>
            {filteredBidderList.map((el: IBidder, idx: number) => (
              ...
              >
                <span aria-label='입찰자 이름' className='text-body1'>{el.bidderNickname}</span>
                <span aria-label='입찰자 입찰가' className='text-body1Bold'>{formatCurrencyWithWon(el.bidAmount)}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      <Layout.Footer type='single'>
        <Button type='button' onClick={() => navigate(-1)} color='cheeseYellow' className='w-full h-full' ariaLabel='확인 완료'>
          확인 완료
        </Button>
      </Layout.Footer>
    </Layout>
