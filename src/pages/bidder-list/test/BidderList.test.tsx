...
describe('입찰자 리스트 페이지 테스트', () => {
  const setup = () => {
    const utils = render(<BidderListMain auctionId={1} />, {
  ...
  test('입찰가를 기준으로 높은 가격 순, 낮은 가격순으로 필터링할 수 있다.', async () => {
    ...
    const getNumber = (str: string) =>
      Number(str.split(' ')[0].replace(/,/g, ''));
    const firstList = screen
      .getAllByLabelText(/입찰가/)
    ...
    const expectedList = [...firstList].sort(
      (a, b) => getNumber(a) - getNumber(b)
    );
    const filteredList = screen
      .getAllByLabelText(/입찰가/)
      .map((el) => el.textContent) as string[];
