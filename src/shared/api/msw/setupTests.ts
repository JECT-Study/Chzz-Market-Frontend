 ...
 * vi.mock을 사용하여 react-router 모듈을 모킹하고, 특정 기능(useNavigate)을 사용자 정의된 모의 함수로 대체
 * 모듈 모킹은 테스트 환경에서 해당 모듈의 실제 구현을 대체하여 테스트 목적에 맞게 수정된 버전을 사용하게 한다.
 * 이 모킹 코드는 테스트 중에 react-router의 useNavigate 훅을 사용하는 컴포넌트를 렌더링할 때 실제 네비게이션 동작을 발생시키지 않고 mockedUseNavigate 라는 모의함수가 호출되도록 한다.
 */
