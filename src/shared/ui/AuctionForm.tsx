...
export const AuctionForm = ({
  ...
  const { mutate: patchPreAuction, isPending: patchPending } =
    usePatchPreAuction();
  ...
  const onPatchSubmit: SubmitHandler<FormFields> = async (data) => {
    ...
    // 유저가 정한 최종 이미지 순서
    const previewImageSequence = images.map((image, idx) => ({
      id: idx + 1,
      image
    }));
    // 새로 삽입한 이미지만 뽑기
    const newImages = previewImageSequence
      .filter((el) => el.image.split(':')[0] === 'data')
      .map((el) => ({ id: el.id, file: convertDataURLtoFile(el.image) }));

    // 기존의 이미지가 현재 어느 위치에 있는지 계산
    const imageSequence = new Map<number, number>();
    existingImages
      .map((el) => {
    ...
    const results = await uploadImagesToS3(
      urls,
      newImages.map((el) => el.file)
    );
  ...
  useEffect(() => {
    if (preAuction) {
      setExistingImages(
        preAuction.images.map((image, idx) => ({ ...image, firstIdx: idx + 1 }))
      );
    }
