...
export const TimeLabel = ({ time }: { time: number }) => {
  ...
  let remainingTime;
  if (formattedTime >= 1) remainingTime = `${Math.ceil(formattedTime)}시간 남음`
  else {
    formattedTime *= 60;
    if (formattedTime >= 1) remainingTime = `${Math.ceil(formattedTime)}분 남음`
    else {
      formattedTime *= 60;
  ...
  return (
    ...
    >
      <p className={`${remainingTime === '경매 종료' && 'text-redNotice'}`}>{remainingTime}</p>
    </div>
  );
};