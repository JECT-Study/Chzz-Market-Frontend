...
describe('경매 상세 조회 테스트', () => {
  ...
  describe.each(auctionDetailsData.slice(0, 3) as IAuctionDetails[])(
    ...
    ({ auctionId, timeRemaining, category }) => {
      test('이미지 최소 1장, 남은 시간, 판매자 이름 및 프로필 사진, 상품 이름, 카테고리, 시작가, 참여 금액, 참여 인원 데이터가 있다.', () => {
        ...
        const sellerName = screen.getByRole('paragraph', { name: /판매자 이름/ });
        const sellerPicture = screen.getByRole('img', { name: /판매자 프로필 사진/ });
      ...
      test('1시간 미만은 빨간색, 16시간 미만은 초록색, 그 이상은 파란색으로 표시된다.', () => {
        ...
        const barColor = hours < 1 ? 'bg-timeColor1' : hours <= 16 ? 'bg-timeColor2' : 'bg-timeColor3';
      ...
      test('카테고리 클릭하면 해당 카테고리 별 목록 페이지로 이동한다.', async () => {
        ...
        expect(mockedUseNavigate).toHaveBeenCalledWith(`/product/list?category=${CATEGORIES[category].lowerCode}`, { state: { category: CATEGORIES[category].value } });
      })
    })
  ...
  describe('진행 중인 경매 테스트', () => {
    ...
    test('구매자이면서 참여하지 않은 사람은 참여하기 버튼만 존재하며, 참여하기 버튼 클릭하면 입찰 페이지로 이동한다.'), async () => {
      const { user } = setup(0)
      ...
      expect(mockedUseNavigate).toHaveBeenCalledWith(`/auctions/bid/${auctionId}`);
    }
    ...
    test('구매자이면서 참여한 사람은 참여 취소 버튼과 금액 수정 및 남은 횟수 버튼이 존재하며, 금액 수정 버튼 클릭하면 입찰 페이지로 이동한다. ', async () => {
      ...
      const remainCount = screen.getByText(`금액 수정 (${remainingBidCount}회 가능)`);
    ...
    test('구매자이면서 참여한 사람은 참여 취소 버튼이 있고, 클릭하면 확인 모달이 나오고, 취소 버튼 클릭하면 참여 취소할 수 있다.', async () => {
      ...
      await user.click(cancelBtn);
      const confirmBtn = screen.getByRole('button', { name: '확인 모달 참여 취소' });
  ...
  describe('종료된 경매 테스트', () => {
    ...
    test('구매자인 경우, 낙찰받은 경매라면 결제하기 버튼이 있고, 클릭하면 결제 페이지로 이동한다.', async () => {
      ...
      await user.click(btn);
      expect(mockedUseNavigate).toHaveBeenCalledWith(`/auctions/${auctionDetailsData[8].auctionId}/payment`);
