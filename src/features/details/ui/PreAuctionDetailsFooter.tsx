...
export const PreAuctionDetailsFooter = ({ isLiked, auctionId, isSeller }: PreAuctionDetailsFooterProps) => {
      ...
      {isSeller
        ...
        <Modal>
          <Modal.Open name='convert'>
            <Button ariaLabel="경매 전환 확인" type='button' className="w-full h-full" color="cheeseYellow">
              경매 전환하기
            </Button>
          </Modal.Open>
          <Modal.Window name='convert'>
            <Confirm type='convert'>
              <Button ariaLabel="경매 전환" type='button' disabled={isPending} loading={isPending} onClick={() => convertToAuction()} className="w-full h-full" color="cheeseYellow">
                전환하기
              </Button>
            </Confirm>
          </Modal.Window>
