  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center h-[10rem] web:h-[12.5rem]">
        <div
          role="status"
          className="w-[2rem] h-[2rem] border-2 border-[#F6F8F8] border-opacity-60 rounded-full size-4 border-b-cheeseYellow animate-spin"
        />
      </div>
    );
  }
      ...
      <div className="flex-1">
        <div className="flex items-start justify-between">
          <div className="flex-col">
            <p className="text-heading3 web:text-heading2">{userNickname}</p>
            {providerType === 'KAKAO' ? (
              <div className="flex w-[8rem] web:w-[11rem] mt-[0.3rem] web:mt-[0.6rem]">
                ...
                >
                  ...
                  <span className="flex-grow text-xs web:text-body1">
                    카카오톡과 연결
                  </span>
                </div>
              </div>
          ...
          <Button
            ...
            onClick={() =>
              navigator('profile/edit', {
                state: { userNickname, userBio, userProfileImageUrl }
              })
            }
          >
            수정
